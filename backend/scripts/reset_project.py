...
pool from `generation/ocean_mask.npz`. Used when we want to restart
...
Usage
-----
    python -m scripts.reset_project [--from generation/ocean_mask.npz]
                                    [--project-day-0 YYYY-MM-DD]

`--from` defaults to `generation/ocean_mask.npz` (the upsampled mask
generated by `generation/upsample_to_backend_mask.py`).
...
DEFAULT_MASK = REPO / "generation" / "ocean_mask.npz"
...
def main() -> None:
    ...
    ap.add_argument("--from", dest="src", type=Path, default=DEFAULT_MASK,
                    help="Mask .npz to load (default: generation/ocean_mask.npz)")
